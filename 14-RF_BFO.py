import numpy as np
from BFOIndividual_rf import BFOIndividual_rf
import random
import copy
...
class BacterialForagingOptimization:
    ...
    def initialize(self):
        ...
        for i in range(0, self.sizepop):
            ind = BFOIndividual_rf(self.vardim, self.bound)
            ind.generate()
    ...
    def chemotaxls(self):
        ...
        for i in range(0, self.sizepop):
            ...
            if i > 0:
                prng = np.random.RandomState(123456789)  # initialize seed
                t = prng.standard_t(i, size=1)
                t = float(t)  # t-distribution
...
bound = np.tile([[1], [200]], 1)
bfo = BacterialForagingOptimization(30, 1, bound,  [2, 2, 50, 4, 50, 0.25, 0.1, 0.2, 0.1, 10])
bfo.solve()